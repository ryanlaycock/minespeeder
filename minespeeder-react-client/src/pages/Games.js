...
function Board({ width, height, id, gameId }) {
  ...
  // TODO Refactor this to be at the game level
  useEffect(() => {
    const interval = setInterval(() => {
      axios.get('http://localhost:8080/v1/games/' + gameId + '/boards/' + id)
        .then(response => {
    ...
    <div className="game">
      {game["boards"].map((board) => (
        <div className="game-board">
          <Board height={board.height} width={board.width} id={board.id} gameId={gameId} />
        </div>
      ))}      
    </div>
  );
